...
Videos.get("/dashboard/:email", async (req, res) => {
  try {
    const { email } = req.params;

    const videosDoc = await videodata.findOne({ email });
    ...
    const totalVideos = videosDoc.VideoData.length;

    const totalViews = videosDoc.VideoData.reduce(

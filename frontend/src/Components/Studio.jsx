...
function Studio() {
      ...
      try {
        if (user?.email) {
          const response = await fetch(
            `${backendURL}/videos/getuservideos/${user?.email}`
          );
  ...
  const ChannelAvailable = async () => {
    try {
      ...
      // EXPECTED: { hasChannel: true/false }
      setisChannel(Boolean(data.hasChannel));
    } catch (error) {
      console.error("Channel check failed", error);
      setisChannel(false); // fallback → allow channel creation
    }
  };
  ...
  // 🔒 Frontend validation
  if (!selectedImage || !ChannelName || !ChannelAbout) {
    ErrorNotify();
    return;
  }
  ...
  try {
    ...
    // 🚀 API CALL (FIXED PATH)
    const response = await fetch(
      `${backendURL}/channel/savechannel`,
      {
        method: "POST",
        headers: {
    ...
    // ✅ Success
    if (response.ok && result.message === "Channel saved successfully") {
      setisChannel(true);
      window.location.reload();
    } 
    // ❌ Backend message (already exists, etc.)
    else {
      alert(result.message || "Channel creation failed");
    }

  } catch (error) {
    console.error("❌ Save channel error:", error);
    alert("Server error while creating channel");
  } finally {
    setisLoading(false);
  }
};
  ...
  // 🔴 1. Prevent publish before video upload finishes
  if (!VideoURL) {
  ...
  try {
    ...
    console.log("📤 Publishing video:", payload);

    // 🔴 4. FIXED API PATH
    const response = await fetch(`${backendURL}/videos/publish`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (response.ok && result === "Published") {
      ...
      alert(result?.message || "Publishing failed");
      setLoading(false);
    }

  } catch (error) {
    console.error("❌ Publish error:", error);
    alert("Server error while publishing video");
    setLoading(false);
  }
};

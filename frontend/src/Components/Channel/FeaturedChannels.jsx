...
function FeaturedChannels(prop) {
      ...
      try {
        if (prop?.newmail) {
          const response = await fetch(
            `${backendURL}/channel/getsubscriptions/${prop?.newmail}`
          );
  ...
  useEffect(() => {
    const getFeaturedChannels = async () => {
      try {
        const response = await fetch(
          `${backendURL}/channel/getfeaturedchannels/${prop?.newmail}`
        );
  ...
  const AddChannel = async () => {
    try {
      if (prop?.newmail && SaveChannel) {
        ...
        const response = await fetch(
          `${backendURL}/channel/savefeaturedchannel/${prop?.newmail}`,
          {
            method: "POST",
  ...
  const DeleteChannel = async (channelid) => {
    try {
      const response = await fetch(
        `${backendURL}/channel/deletefeaturedchannel/${user?.email}/${channelid}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
